...
from app.models import User, Requirement, Resource, Contract
...
def get_client_stats(
    ...
    open_requirements = [r for r in requirements if r.status.value == "Open"]
    closed_requirements = [r for r in requirements if r.status.value == "Closed"]
    
    # Count total matching profiles
    total_matches = 0
    for req in requirements:
        match_count = db.query(Requirement).filter(Requirement.id == req.id).count()
...
def get_vendor_stats(
    ...
    active_resources = [r for r in resources if r.status.value == "Available"]
    ...
    completed_contracts = [c for c in contracts if c.status.value == "Completed"]
    
    # Calculate monthly revenue from active contracts
    monthly_revenue = sum(c.rate for c in active_contracts if c.billing_cycle.value == "Monthly")
