...
import {
  ...
  Sparkles, Shield, Bell, HelpCircle, Crown
} from 'lucide-react';
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <div className={`flex items-center gap-3 p-4 border-b border-slate-200/60 dark:border-slate-700/60 ${isCollapsed ? 'justify-center' : ''}`}>
        <div className="w-10 h-10 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 flex items-center justify-center text-white font-bold text-lg shadow-lg shadow-indigo-500/30 flex-shrink-0">
          <Shield size={20} />
        </div>
        {!isCollapsed && (
          <div>
            <span className="text-lg font-bold text-slate-800 dark:text-slate-100">Admin</span>
            <span className="block text-[10px] text-slate-500 dark:text-slate-400">Control Panel</span>
          </div>
        )}
      </div>
