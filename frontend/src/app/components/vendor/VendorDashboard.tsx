    ...
    <div className="space-y-8">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 sm:p-6 border">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-5 sm:mb-6">
          <div><h2 className="text-lg sm:text-xl font-bold">Resource Availability Trend</h2><p className="text-sm text-slate-500 mt-1">{trendFilter.charAt(0).toUpperCase() + trendFilter.slice(1)} resource utilization overview</p></div>
          <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-700 rounded-xl p-1 self-start flex-shrink-0">
            {(['weekly', 'monthly', 'yearly'] as const).map((filter) => (
              <button key={filter} onClick={() => setTrendFilter(filter)} className={`px-2.5 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-medium rounded-lg transition-all ${trendFilter === filter ? 'bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-lg' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>
