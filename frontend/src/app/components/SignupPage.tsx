// components/SignupPage.tsx - Enhanced with Role Selection
import { useState, useEffect } from 'react';
import React from 'react'
import { Globe, Building2, Mail, Phone, User, Upload, ArrowRight, ArrowLeft, Check, Lock, Eye, EyeOff, PartyPopper, Sparkles, Shield, Award } from 'lucide-react';
...
export function SignupPage({ onSignup, onBackToLogin, onBackToHome }: SignupPageProps) {
  ...
  // Show role modal when signup is triggered
  useEffect(() => {
          ...
          <div className="flex-1 flex items-center justify-center py-6 px-6">
            ...
            <div className="w-full max-w-md">
              ...
              <div className="text-center mb-5">
                ...
                {formData.role && (
                  <div className={`inline-flex items-center gap-1.5 mt-1 px-3 py-1 rounded-full text-xs font-medium ${formData.role === 'client'
                    ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                    : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'
                    }`}>
                    {formData.role === 'client' ? '🏢 Client' : '🤝 Vendor'}
                  </div>
                )}
              </div>
